...
import { createBillForObd } from '@/services/api/billingAPI';
...
const CreateOpdPatient = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Step 2 — Create bill for all selected tests
      ...
      await createBillForObd(newPatientId, { itemDetail });

      // Create investigation request for the tests
      const investigationData = {
        obdPatientId: newPatientId,
        type: 'lab',
        priority,
    ...
    <CashierLayout>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => navigate('/cashier/opd-patients')}
            className="btn btn-ghost" disabled={isLoading}>
            Cancel
          </button>
