...
const EditConsultation = () => {
  ...
  // load patient and medical-record templates
  useEffect(() => {
    let mounted = true;
    const load = async () => {
      try {
        // Fetch Medical Records categories
        try {
  ...
  // load existing consultation values when editing
  useEffect(() => {
    ...
    const loadConsult = async () => {
      try {
        ...
        if (mounted && data) {
          ...
          setHistoryOfPresentingComplaint(data.complaintHistory || "");
          setDiagnosis(data.diagnosis || "");

          // map arrays back to form state shapes
          setComplaints((data.complaint || []).map(c => ({
            name: c.symptom,
    ...
    toast.promise(
      updateConsultation(consultationId, payload),
      {
        ...
        success: (res) => {
          ...
          // after update go back to medical history
        ...
        error: (err) => {
          ...
          return err?.response?.data?.message || 'Failed to update consultation';
        }
      }
    ).finally(() => setSaving(false));
  };

  const title = consultationId ? 'Edit Consultation' : 'Add New Consultation';
  ...
  return (
    <div className="flex h-screen bg-base-200/50">
        ...
        <div className="flex overflow-y-auto flex-col p-4 sm:p-6 space-y-6">
          ...
          <div className="flex justify-center gap-4 pt-4 pb-12">
            <button
              ...
              disabled={saving}
            >
              {consultationId ? "Update Consultation" : "Save Now"}
            </button>
          </div>
        </div>
      </div>
      ...
      <ConfirmationModal
        ...
        title={consultationId ? "Update Consultation" : "Save Consultation"}
        message={consultationId ? "Are you sure you want to update this consultation?" : "Are you sure you want to save this consultation? This action cannot be undone."}
        confirmText={consultationId ? "Update Consultation" : "Save Consultation"}
        cancelText="Cancel"
      />
    </div>
  );
