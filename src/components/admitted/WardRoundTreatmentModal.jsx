...
import { getMedicationDispenseSlots } from '@/services/api/dispensesAPI'
...
const WardRoundTreatmentModal = ({ isOpen, onClose, admissionId, prescriptions = [], readOnly = false, onCompleted }) => {
  const [days, setDays] = useState(3)
  const [checked, setChecked] = useState({})
  const [administrationTimes, setAdministrationTimes] = useState({})
  const [dispensedSlots, setDispensedSlots] = useState({})
  ...
  useEffect(() => {
    ...
    Promise.all([
      getMedicationAdministrations(admissionId),
      getMedicationDispenseSlots(admissionId)
    ])
      .then(([adminRecords, dispenseRecords]) => {
        const nextChecked = {}
        const nextTimes = {}
        ;(Array.isArray(adminRecords) ? adminRecords : []).forEach((record) => {
          const noteMatch = String(record.notes || '').match(/Administered ([^,]+), day (\d+)/i)
          ...
          nextTimes[key] = record.administeredAt
        })
        setChecked(nextChecked)
        setAdministrationTimes(nextTimes)

        const nextDispensed = {}
        ;(Array.isArray(dispenseRecords) ? dispenseRecords : []).forEach((record) => {
          const noteMatch = String(record.notes || '').match(/Dispensed ([^,]+), day (\d+)/i)
          const medicationKey = `${record.prescriptionId}-${record.medicationId}`
          if (!noteMatch) return
          const key = `${medicationKey}-${noteMatch[2]}-${noteMatch[1]}`
          nextDispensed[key] = true
        })
        setDispensedSlots(nextDispensed)
      })
      .catch((error) => console.error('WardRoundTreatmentModal: failed to load logs', error))
  ...
  const handleBoxClick = async (item, day, slotIndex, slot) => {
    ...
    if (!dispensedSlots[key]) {
      return toast.error('Cannot administer: Pharmacist has not dispensed this dose yet.')
    }
    ...
    try {
      await administerMedication(admissionId, {
        prescriptionId: item.prescriptionId,
        ...(item.medicationId ? { medicationId: item.medicationId } : { medicationIndex: item.medicationIndex }),
        doseGiven: getDose(item.medication),
        notes: `Administered ${slot}, day ${day}${notes.trim() ? ` - ${notes.trim()}` : ''}`,
        administeredAt: new Date().toISOString(),
      })
      setChecked((current) => ({ ...current, [key]: true }))
      setAdministrationTimes((current) => ({ ...current, [key]: new Date().toISOString() }))
      toast.success(`${item.medication.drugName} logged — ${slot}, day ${day}`)
      onCompleted && onCompleted()
    } catch (error) {
      console.error('WardRoundTreatmentModal: administration error', error)
      toast.error(error?.response?.data?.error || error?.message || 'Failed to log dose')
    } finally {
      setSavingKeys((prev) => {
  ...
  const handleGenerateBill = async () => {
    toast.error('Billing is now generated by the Pharmacist upon dispensing.')
  }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-5xl max-h-[92vh] overflow-hidden bg-base-100 rounded-2xl shadow-2xl border border-base-200 flex flex-col">
        <div className="flex items-center justify-between gap-4 p-5 border-b border-base-200">
          <div>
            <h3 className="text-lg font-bold text-base-content">Treatment Chart</h3>
            <p className="text-xs text-base-content/60">
              {readOnly
                ? 'Viewing logged doses.'
                : 'Tick a box to log that dose now. Each tick saves instantly — boxes beyond the prescribed frequency are locked. You can only log doses that have been dispensed by the Pharmacist.'}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-auto p-5 space-y-4">
                  ...
                  return (
                    <tr key={item.key}>
                      ...
                      {Array.from({ length: days }, (_, dayIndex) => TIME_SLOTS.map((slot, slotIndex) => {
                        ...
                        const canAdminister = dispensedSlots[key]
                        const isChecked = Boolean(checked[key])
                        const isSaving = savingKeys.has(key)
                        const dayQuotaMet = countCheckedForDay(item, day, frequencyCount) >= frequencyCount
                        const disabled = readOnly || !eligible || isChecked || isSaving || (dayQuotaMet && !isChecked) || !canAdminister
                            ...
                            {eligible ? (
                              <div className="flex flex-col items-center">
                                <div 
                                  ...
                                  data-tip={isChecked ? `Logged at ${new Date(administrationTimes[key]).toLocaleTimeString()}` : !canAdminister ? "Awaiting pharmacist dispense" : `Click to log ${slot} dose`}
                                >
                                  <input
                                    type="checkbox"
                                    className={`checkbox checkbox-primary checkbox-sm ${!canAdminister ? 'opacity-30 cursor-not-allowed' : ''}`}
                                    checked={isChecked}
                                    disabled={disabled}
          ...
          {!readOnly && (
            <textarea
              ...
              placeholder="General administration notes (optional — applied to the next dose you log)"
              rows={2}
            />
          )}
...
export default WardRoundTreatmentModal